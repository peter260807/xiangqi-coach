...
- **剪枝（LMR + 空着裁剪）已实现但默认关闭**（2026-09-23）：深度收益是真的（同 1.5 秒
  6 层 → 9 层，固定深度节点数 −68%），但 40 局归因 A/B 是 47.5%、Elo −17、区间跨 0 ——
  **搜得更深没有换来棋力**，和 P1-2 是同一类现象。而且**空着裁剪在这个引擎里几乎不起作用**：
  单独开启节点数不减反增（22622 → 22922），与 LMR 同开时节点数也不变。
  做成 opt-in（`XQ_LMR=1` / `XQ_NULL=1`），等大样本 A/B 有正证据再翻默认值
