...
> 引擎规格三种：`uci:<路径>`（本项目 Swift 引擎经 UCI 前端，或 Pikafish）、
...
- ⚠️ **搜索与对局层对「长将」的判断不一致（已知缺口，未修）**：搜索内把重复局面一律按
  **和棋**算（P2-2），对局层却对**长将**判该方**负**。两者对不上 → 引擎在搜索里把长将循环
  当成「捞到半分」而**主动走进去**，到对局层却被判负：棋力上白送一局，测量上污染 A/B
  （实测一批 7 局里占 2 局）。建议做法：搜索里遇到重复时，若造成重复的那一手是将军，
  就按长将方负给分，而不是 0（两批对局实测都受它污染：7 局批占 2 局、16 局批又占 2 局，
  且都是优势方主动走进长将循环 —— 结构性污染，不是偶发）
